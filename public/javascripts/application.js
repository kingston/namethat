...
function loggedIn(session) {
  $("#fb-login").hide();
  initChallenge(session);
}

var appstate = {};

function initChallenge(session) {
  ...
  function(response) {
    if (response.length == 0) {
      alert("We couldn't load any friends.  Please try again or try another time.  Sorry!");
      return;
    }
...
function checkAnswer() {
  var answer = removeDiacritics($("#answer").val().toLowerCase());
  if (answer.length < 3) {
  ...
  var correct = true;
  var normalizedName = removeDiacritics(name.toLowerCase()); 
